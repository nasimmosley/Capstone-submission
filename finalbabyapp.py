...
with data:
    ...
    with col1:
        ...
        fig.update_layout(width = 700, height=600, xaxis = dict(title = 'Year', showticklabels=True), yaxis=dict(title="Count" , showticklabels=True))
        st.plotly_chart(fig)
        ...
        def make_new_names(str1, str2):
            ...
            for name in big_names_list:
                if len(name)< 9:
                    ...
                    play_name = name.rstrip(name[-1])
                    new_name = play_name[:len(play_name)//2], play_name[len(play_name)//2:]
                    new_list.append(new_name)
            flat_list = [item for sublist in new_list for item in sublist]
            new_flat_list=[]
            for word in flat_list:
                new_flat_list.append(word.capitalize())
            return sorted(set(new_flat_list))
